...
class DatabaseUser extends Database {
	...
	*/
	public function getUserByUsername($username) {
		$sqlQuery = sprintf("SELECT ".$this->getRows()." FROM user WHERE username = '%s'", $this->escapeString($username) );
	...
	*/
	public function getUserById($userId) {
		$sqlQuery = sprintf("SELECT ".$this->getRows()." FROM user WHERE id = %u", $this->escapeString($userId) );
		$result = $this->getDb()->query($sqlQuery);
		if($result->num_rows > 0) {
			return $result;
		} else {
